...
import { PackageCardStacked } from "@/components/packages/PackageCardStacked";
import { PackageCardPhoto } from "@/components/packages/PackageCardPhoto";
...
export default function Home() {
  return (
    <>
      ...
      {/* Diseño 2 — misma sección, card con fotos apiladas (borrador para comparar) */}
      <section className="bg-brand-white">
        <PackagesIntro />
        <div className="mx-auto grid max-w-6xl gap-8 px-6 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURED_PACKAGES.map((pkg) => (
            <PackageCardStacked key={pkg.slug} pkg={pkg} />
          ))}
        </div>
        <div className="flex justify-center px-6 pt-10 pb-20">
          <Link
      ...
      {/* Diseño 3 — misma sección, card foto full-bleed con degradado (borrador para comparar) */}
      <section className="bg-white">
        <PackagesIntro />
        <div className="mx-auto grid max-w-6xl gap-8 px-6 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURED_PACKAGES.map((pkg) => (
            <PackageCardPhoto key={pkg.slug} pkg={pkg} />
          ))}
        </div>
        <div className="flex justify-center px-6 pt-10 pb-20">
          <Link
